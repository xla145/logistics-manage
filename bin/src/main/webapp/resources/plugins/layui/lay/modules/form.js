  ...
  ,Form = function(){
    this.config = {
      verify: {
        ...
        ,number: [/^\d+$/,'只能填写数字']
    	,loginName: [/^\w{5,12}$/,'登录名只能是数字、字母和下划线组合，且5-12位']
    	,password: [/^(\w|\-|\d|\+|\.){6,12}$/,'密码只能是英文字母、数字、下划线、横线、加号及点号组成，且6-12位']
        ,money: [/^([1-9][\d]{0,7}|0)(\.[\d]{1,2})?$/,'金额格式不正确']
  ...
  Form.prototype.render = function(type){
    var that = this, items = {
      ...
      ,flexTable : function(){
        ...
        //   //先丢进去活动表格
        //   tableFloat.append(floatBox.append($table.clone()));
        //   tableWrap.append(tableFloat);
        //   $twrap.empty().append(tableWrap)
        ...
        //   $table.width('99%')
        //   floatBox.width('100%');
        //   $table.css('min-width',$tableWidth)
        //   floatBox.css('min-width',$tableWidth+10)

        //   tableLeft.append(leftBox.append($table.clone().width($table.outerWidth(true))));
        //   tableWrap.append(tableLeft);

        //   tableRight.append(rightBox.append($table.clone().width($table.outerWidth(true))));
        //   tableWrap.append(tableRight);

        //   $twrap.append(tableWrap)
