 ...
 */
@Controller
@RequestMapping(value = "/supplier/activity")
public class SupplierActivityController extends BaseController{
...
//		model.addAttribute("supplierType", ProductGroupTypeConstant.values());
		...
		return "modules/supplier/activity/index";
	}
...
//		Integer type = ReqUtils.getParamToInteger(request, "type", null);
		...
		conn.add(new Conditions("product_group_type", SqlExpr.EQUAL, ProductGroupTypeConstant.ACTIVITY.getId()), SqlJoin.AND);

		PagePojo<Supplier> page = supplierService.getSupplierPage(conn, pageNo, pageSize);
		//render结果
		return PageUtils.render(request, "WEB-INF/view/modules/supplier/activity", "_list.html", page);
	}
...
//		}
		supplier.setProductGroupType(ProductGroupTypeConstant.ACTIVITY.getId());
...
//	@Auth(id = 4)
	...
	public String editView(HttpServletRequest request, HttpServletResponse response){
		...
		if (id != null) {
			...
			PageUtils.render(request, response, "WEB-INF/view/modules/supplier/activity", "_edit.html", supplier);
		}
		return null;
	}
...
//	@Auth(id = 5)
	 ...
	 * @return
	 */
	@RequestMapping(value = "/productActivityList")
	@ResponseBody
	public JSONObject productActivityList(HttpServletRequest request){
		//参数获取
		int pageNo = ReqUtils.getParamToInt(request, "pageNo", 1);
		int pageSize = ReqUtils.getParamToInt(request, "pageSize", 15);
		Integer supplierId = ReqUtils.getParamToInteger(request, "supplierId", null);
		PagePojo<ProductActivityModel> page = productService.getProductActivityBySupplierId(supplierId, pageNo, pageSize);
		//render结果
		return PageUtils.render(request, "WEB-INF/view/modules/supplier/activity", "activity_list.html", page);
	}
	...
	public String productIndex(HttpServletRequest request,Model model){
		...
		return "modules/supplier/activity/activity_index";
	} 
