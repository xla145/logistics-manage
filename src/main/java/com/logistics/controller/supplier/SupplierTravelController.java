 ...
 */
@Controller
@RequestMapping(value = "/supplier/travel")
public class SupplierTravelController extends BaseController{
...
//		model.addAttribute("supplierType", ProductGroupTypeConstant.values());
		model.addAttribute("settlementTypeList", SettlementTypeConstant.values());
		return "modules/supplier/travel/index";
	}
...
//		conn.add(new Conditions("group_type", SqlExpr.EQUAL, type), SqlJoin.AND);
		conn.add(new Conditions("product_group_type", SqlExpr.EQUAL, ProductGroupTypeConstant.TRAVEL.getId()), SqlJoin.AND);

		PagePojo<Supplier> page = supplierService.getSupplierPage(conn, pageNo, pageSize);
		//render结果
		return PageUtils.render(request, "WEB-INF/view/modules/supplier/travel", "_list.html", page);
	}
	...
	public JSONObject save(HttpServletRequest request, Supplier supplier){
		...
		supplier.setProductGroupType(ProductGroupTypeConstant.TRAVEL.getId());
...
//	@Auth(id = 4)
	...
	public String editView(HttpServletRequest request, HttpServletResponse response){
		...
		if (id != null) {
			...
			PageUtils.render(request, response, "WEB-INF/view/modules/supplier/travel/", "_edit.html", supplier);
		}
		return null;
	}
...
//	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/productTravelList")
	@ResponseBody
	public JSONObject productGoodsList(HttpServletRequest request){
		...
		int pageSize = ReqUtils.getParamToInt(request, "pageSize", 15);
		Integer supplierId = ReqUtils.getParamToInteger(request, "supplierId", null);
		PagePojo<ProductTravelModel> page = productService.getProductTravelBySupplierId(supplierId, pageNo, pageSize);
		//render结果
		return PageUtils.render(request, "WEB-INF/view/modules/supplier/travel/", "travel_list.html", page);
	}
	...
	public String productIndex(HttpServletRequest request,Model model){
		//参数获取请求的产品类型名称
		Integer type = ReqUtils.getParamToInt(request, "type", 1);
		Integer supplierId = ReqUtils.getParamToInt(request, "id", 1);
		//render结果
		model.addAttribute("supplierId", supplierId);
		model.addAttribute("type", type);
		return "modules/supplier/travel/"+ ProductGroupTypeConstant.getProductTypeById(type).getIndex();
	} 
