 ...
 */
@Controller
@RequestMapping("/travel/live/order")
public class TravelLiveOrderController extends BaseController{

	@Autowired
	private ITravelLiveOrderService travelLiveOrderService;
	@Autowired
	private ITravelLiveRefundService iTravelLiveRefundService;
	@Autowired
	private IProductTravelLiveService productTravelLiveService;
	@Autowired
	...
	public String tempView(HttpServletRequest request, Model model){
		List<ProductTravelSummary> list = productTravelLiveService.getTravelSummaryList(ProductConstant.PRODUCT_STATUS_ONOFFER);
		model.addAttribute("travelSummary", list);
		return "/modules/travel/travel-live/order/index";
	}
	...
	public JSONObject tempList(HttpServletRequest request, OrderTravelSummary orderTravelTemp){
		...
		PagePojo<OrderTravelSummary> page = travelLiveOrderService.getOrderTravel(map, pageNo, pageSize);
	...
	public JSONObject tempAdd(HttpServletRequest request, OrderTravelModel orderTravelModel){
		...
		RecordBean<OrderTravelModel> recordBean = travelLiveOrderService.addOrderTravel(pid,orderTravelPersonnels,orderTravelModel,operName,operId);
	...
	public String editView(HttpServletRequest request, Model model){
		...
		OrderTravelSummary orderTravelSummary = travelLiveOrderService.getOrderTravel(orderId);
		List<ProductTravelSummary> list = productTravelLiveService.getTravelSummaryList(null); // 传null代表筛选全部
	...
	public JSONObject getPersonnels(HttpServletRequest request, Model model){
		String orderId = request.getParameter("orderId");
		List<OrderTravelPersonnelModel>  orderTravelPersonnel = travelLiveOrderService.getOrderTravelPersonnelModel(orderId);
	...
	public String getPersonView(HttpServletRequest request, OrderTravelPersonnelModel orderTravelPersonnel,Model model){
		...
		if (id != null) {
			model.addAttribute("data",travelLiveOrderService.getOrderTravelPersonnelModel(id));
		} else {
	...
	public JSONObject editPerson(HttpServletRequest request, OrderTravelPersonnel orderTravelPersonnel){
		RecordBean<OrderTravelPersonnel> result = travelLiveOrderService.editOrderTravelPersonnel(orderTravelPersonnel);
	...
	public JSONObject editTemp(HttpServletRequest request, Model model, OrderTravelModel orderTravelModel){
		...
		List<OrderTravelPersonnel> orderTravelPersonnels = JSON.parseArray(personnels, OrderTravelPersonnel.class);
		RecordBean<OrderTravelModel> recordBean = travelLiveOrderService.updateOrderTravel(pid,orderTravelPersonnels,orderTravelModel);
		if (recordBean.isSuccessCode()) {
	...
	public JSONObject payList(HttpServletRequest request){
		String orderId = ReqUtils.getParam(request,"orderId",null);
		List<PayInfo> payOrderList = travelLiveOrderService.getPayOrderByOrderId(orderId);
	...
	public JSONObject getPayRefundOrderList(HttpServletRequest request){
		String orderId = request.getParameter("orderId");
		List<PayRefundOrder>  payRefundOrderList = iTravelLiveRefundService.getPayRefundByOrderId(orderId);
...
//		Integer payType = ReqUtils.getParamToInteger(request,"payType",null);
		...
		String operName = CommonUtil.getContentFromJSON(session.getAttribute(BaseConstant.SYS_USER),"name");
		Integer operId = (Integer) session.getAttribute(BaseConstant.SYS_UID);
		RecordBean<String> recordBean = travelLiveOrderService.proceeds(orderId,payAmount,payWay,remark,operId,operName);
		if (recordBean.isSuccessCode()) {
	...
	public JSONObject refundOrder(HttpServletRequest request,PayRefundOrder payRefundOrder){
		...
		String operName = CommonUtil.getContentFromJSON(session.getAttribute(BaseConstant.SYS_USER),"name");
		Integer operId = (Integer) session.getAttribute(BaseConstant.SYS_UID);
		RecordBean<String> result = iTravelLiveRefundService.refund(payRefundOrder.getOrderId(),payRefundOrder.getAmount(),payRefundOrder.getApplyReason(),payRefundOrder.getRemark(),operId,operName,isRefundCoupon);
		if (result.isSuccessCode()) {
			return JsonBean.success(result.getMsg());
		}
		return JsonBean.error(result.getMsg());
	...
	public JSONObject finish(HttpServletRequest request){
		...
		String orderId = ReqUtils.getParam(request,"orderId",null);
//		String remark = ReqUtils.getParam(request,"remark",null);
		RecordBean<String> result = travelLiveOrderService.orderSuccess(orderId,null,operId,operName);
		if (result.isSuccessCode()) {
	...
	public JSONObject reduce(HttpServletRequest request){
		...
		boolean result = travelLiveOrderService.orderCancel(orderId,reason,operId,operName,ip);
		if (result) {
			return JsonBean.success("成功！");
		}
	...
	public JSONObject reduceAndRefundOrder(HttpServletRequest request,PayRefundOrder payRefundOrder){
		...
		Integer operId = (Integer) session.getAttribute(BaseConstant.SYS_UID);
		String IP = CommonUtil.getIp(request);
		RecordBean<PayRefundOrder> result = travelLiveOrderService.reduceAndRefundOrder(payRefundOrder,operId,operName,IP,isRefundCoupon);
		if (result.isSuccessCode()) {
			return JsonBean.success("成功！");
		}
		return JsonBean.error(result.getMsg());
	...
	public JSONObject export(HttpServletResponse response, HttpServletRequest request) {
		...
		try {
			...
			List<OrderTravelSummary> list = travelLiveOrderService.orderTravelSummaryList(map);
	...
	public String orderCouponInfo(HttpServletRequest request,Model model){
		String orderId = ReqUtils.getParam(request,"orderId",null);
		OrderTravelSummary orderTravelSummary = travelLiveOrderService.getOrderTravelSummary(orderId);
	...
	public JSONObject getCoupon(HttpServletRequest request){
		String orderId = ReqUtils.getParam(request,"orderId",null);
		List<MemberCouponModel> memberCouponList = travelLiveOrderService.getMemberCouponModel(orderId);
