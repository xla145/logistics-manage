...
public class TravelProductController extends BaseController {
	...
	public JSONObject list(HttpServletRequest request){
		...
		PagePojo<ProductTravelModel> page = iProductTravelService.productTravelPage(map, pageNo, pageSize);
	...
	public JSONObject add(HttpServletRequest request, ProductTravelModel productTravelModel){
		...
		RecordBean<String> productTravelModelDataObj = iProductTravelService.addProductTravel(productTravelModel);
	...
	public String editView(HttpServletRequest request, Model model) {
		...
		if (pid != null) {
			travelModel = iProductTravelService.getProductTravel(pid);
			model.addAttribute("data", travelModel);
			return "modules/travel/_edit";
		} else {
			String id = "YT-" + DateUtil.formatDate(new Date(), "YYMM") + CommonUtil.getNumberRandom(4);
			model.addAttribute("pid", id);
			return "modules/travel/_add";
		}
	}
...
//		model.addAttribute("data", travelModel);
	...
	public JSONObject edit(HttpServletRequest request, ProductTravelModel productTravelModel){
		...
		DataObj<ProductTravelModel> productTravelModelDataObj = iProductTravelService.updateProductTravel(productTravelModel);
		if (productTravelModelDataObj.isSuccessCode()) {
			return JsonBean.success(productTravelModelDataObj.getMsg());
		}
		return JsonBean.error(productTravelModelDataObj.getMsg());
	...
	public JSONObject upOrDownProduct(HttpServletRequest request){
		...
		DataObj<String> result = iProductTravelService.upOrDownProductTravel(ids,status);
	...
	public JSONObject del(HttpServletRequest request){
		String[] ids = request.getParameterValues("ids");
		DataObj<String> result = iProductTravelService.delProductTravel(ids);
		if (result.isSuccessCode()) {
			return JsonBean.success(result.getMsg());
		}
		return JsonBean.error(result.getMsg());
	...
	public JSONObject copyProduct(HttpServletRequest request){
		String pid = ReqUtils.getParam(request,"pid",null);
		RecordBean recordBean = iProductTravelService.copyProduct(pid);
